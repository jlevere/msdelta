...
fn managed_cli_readiness_counts_are_explicit() {
    ...
    assert_eq!(count_by(&cli_rows, |row| row.status, "partial"), 21);
    assert_eq!(count_by(&cli_rows, |row| row.status, "missing"), 1);
...
fn managed_workstream_readiness_counts_are_explicit() {
    ...
    assert_eq!(count_by(&rows, |row| row.status, "partial"), 21);
    assert_eq!(count_by(&rows, |row| row.status, "missing"), 8);
    ...
    assert_eq!(count_by(&rows, |row| row.oracle_level, "unit"), 16);
    assert_eq!(count_by(&rows, |row| row.oracle_level, "needs_fixture"), 9);
}
