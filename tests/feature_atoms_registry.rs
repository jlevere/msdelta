...
fn managed_cli_readiness_counts_are_explicit() {
    ...
    assert_eq!(count_by(&cli_rows, |row| row.status, "partial"), 13);
    assert_eq!(count_by(&cli_rows, |row| row.status, "missing"), 9);
...
fn managed_workstream_readiness_counts_are_explicit() {
    ...
    assert_eq!(count_by(&rows, |row| row.status, "partial"), 13);
    assert_eq!(count_by(&rows, |row| row.status, "missing"), 16);
    ...
    assert_eq!(count_by(&rows, |row| row.oracle_level, "needs_fixture"), 17);
}
