...
fn main() {
    let dir = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "notes/genuine-samples/sweep".into());
    ...
    for line in manifest.lines() {
        ...
        match msdelta::pa30::apply(&reference, &delta) {
            ...
            Ok(out) => {
                fail += 1;
                println!(
                    "FAIL hash-mismatch id={id} base_len={} got_len={} delta_len={}",
                    f[3],
                    out.len(),
                    delta.len()
                );
            ...
            Err(e) => {
                fail += 1;
                println!(
                    "FAIL err id={id} base_len={} delta_len={}: {e}",
                    f[3],
                    delta.len()
                );
                let k = format!("ERR {e}");
